...
func MaterialSearchByKey(key string, car, place, page, perPage int) ([]Material, int64, error) {
	...
	}).Where("place_id = ? or 0 = ?", place, place).Where("user_id in ?", userId).Where("name LIKE ? OR model LIKE ?", regKey, regKey).Offset((page - 1) * perPage).Limit(perPage).Order("id DESC").Find(&materials).Error
	if err != nil {
		fmt.Println(err)
		return []Material{}, 0, err
	}
	...
	var count int64 = 0
	err = GlobalDb.Model(Material{}).Where("name LIKE ? OR model LIKE ?", regKey, regKey).Count(&count).Error
	if err != nil {
		fmt.Println(err)
		return []Material{}, 0, err
	}
